...
import { User, UserRole, BitrixConfig } from '../types';
import { 
  UserPlus, Shield, User as UserIcon, Mail, 
  MoreVertical, RefreshCw, Share2, Briefcase, 
  CheckCircle2, Key, Lock, Unlock, X, Star, Coins
} from 'lucide-react';
...
const UsersManagement: React.FC<UsersManagementProps> = ({ staff, onSync, isBitrixEnabled, bitrixConfig, onToggleProduction, onUpdateStaff }) => {
  ...
  const saveSalary = (userId: string) => {
    onUpdateStaff(userId, { shiftSalary: parseInt(newSalary) || 0 });
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {staff.map((member) => (
          <div key={member.id} className={`bg-white p-6 rounded-2xl border transition-all relative overflow-hidden group ${
            ...
            <div className="mt-6 flex flex-col gap-2 relative z-20">
              <div className="flex gap-2">
                ...
                {bitrixConfig?.paymentFormat === 'salary' && (
                  <button 
                    onClick={() => { setEditingSalary(member.id); setNewSalary(String(member.shiftSalary || 0)); }}
                    className={`px-3 py-2 rounded-xl transition-all ${member.shiftSalary ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
            ...
            {editingSalary === member.id && (
              <div className="absolute inset-0 z-30 bg-white p-6 flex flex-col animate-in fade-in zoom-in duration-200">
                <div className="flex justify-between items-center mb-4">
                   <h4 className="text-xs font-black uppercase tracking-widest text-slate-400">Ставка за смену</h4>
                   <button onClick={() => setEditingSalary(null)} className="text-slate-400 hover:text-slate-600"><X size={16}/></button>
                </div>
                <div className="space-y-4">
                  <input 
                    type="number" 
                    ...
                    autoFocus
                  />
                  <p className="text-[10px] text-slate-400 italic">Укажите сумму, которую сотрудник получает за одну отработанную смену.</p>
                  <button 
                    onClick={() => saveSalary(member.id)}
                    className="w-full bg-emerald-600 text-white py-3 rounded-xl font-bold text-xs uppercase tracking-widest shadow-lg shadow-emerald-500/20"
                  >
                    Сохранить ставку
                  </button>
                </div>
