<?php
/**
 * CLOUD API FOR MEBELPLAN ERP (v2.6)
 * PostgreSQL SSL Connection Utility
 */

// 1. Максимальное подавление вывода ошибок в текст
ini_set('display_errors', 0);
ini_set('display_startup_errors', 0);
error_reporting(0); // Временно отключаем отчеты, чтобы не портить JSON
...
// Запускаем буферизацию, чтобы перехватить случайные выводы
ob_start();
...
function sendJson($data, $code = 200) {
    // Очищаем всё, что могло попасть в буфер до этого (ошибки, варнинги)
    while (ob_get_level()) {
        ob_end_clean();
    }
...
try {
    // 4. Соединение с БД
    // Если сертификат не скачался, пробуем без sslmode=verify-full (но с sslmode=require)
    $ssl_mode = (file_exists($cert_path) && filesize($cert_path) > 0) ? "verify-full" : "require";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'test') {
            sendJson(['success' => true, 'message' => 'API работает. База подключена.']);
        } elseif ($action === 'load') {
